...
int main(int argc, char *argv[])
{
	...
	sprintf(swp_file_name, "%s/test_file.swp", path);
	...
	/* Copy the file (Create swp file) */
	swp_fd = open(swp_file_name, O_CREAT|O_RDWR, 0755);
	if (swp_fd < 0) {
		printf("open() error during open swp file\n");
		close(orig_fd);
		exit(-1);
	}

	while (written < file_size) {
		ret = write(swp_fd, buf, MAX_BUF);
		if (ret < 0) {
			printf("write() error during create swp file\n");
			close(orig_fd);
			close(swp_fd);
			exit(-1);
		}
	...
	for (i = 0; i<num_pages; i++) {
		lseek(swp_fd, offsets[i], SEEK_SET);
		ret = write(swp_fd, buf, WRITE_SIZE);
		if (ret < 0) {
	...
	ret = rename(swp_file_name, orig_file_name);
	if (ret < 0) {
		printf("renameat2() error\n");
		close(orig_fd);
		close(swp_fd);
		close(dir_fd);
		exit(-1);
