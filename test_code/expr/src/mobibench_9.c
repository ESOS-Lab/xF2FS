...
//	printf("db thread start\n");
	...
	for (i = 0; i < 9; i++)
		sprintf(filename[i], "%s/test.db%d", pathname, i);
//	printf("open db : %s\n", filename);

	for (i = 0; i < 9; i++) {
		rc = sqlite3_open(filename[i], &db[i]);
		...
		if(db_init_show == 0)
		{
			...
			printf("-----------------------------------------\n");
			exec_sql(db[i], "select sqlite_version() AS sqlite_version;", sql_cb);
			exec_sql(db[i], "PRAGMA page_size;", sql_cb);
			exec_sql(db[i], "PRAGMA journal_mode;", sql_cb);
			exec_sql(db[i], "PRAGMA synchronous;", sql_cb);		
		}

		if(db_mode == 0)
		{
			exec_sql(db[i], "DROP TABLE IF EXISTS tblMyList;", NULL);
		}

		exec_sql(db[i], "CREATE TABLE IF NOT EXISTS tblMyList (id INTEGER PRIMARY KEY autoincrement, Value TEXT not null, creation_date long);", NULL);
		...
		if(db_mode != 0)
		{
			...
			rc = sqlite3_get_table(db[i], sql, &result, &rows, &columns, &errmsg);
			...
			{
				init_db_for_update(db[i], filename, db_transactions - column_count);
			}
		}
	}
	...
	exec_sql(db[0], sql, NULL);
	sprintf(sql, "ATTACH DATABASE '%s/test.db5' AS 'db5'", pathname);
	exec_sql(db[0], sql, NULL);
	sprintf(sql, "PRAGMA db5.journal_mode=%s", get_journal_mode_string(db_journal_mode));
	exec_sql(db[0], sql, NULL);
	sprintf(sql, "ATTACH DATABASE '%s/test.db6' AS 'db6'", pathname);
	exec_sql(db[0], sql, NULL);
	sprintf(sql, "PRAGMA db6.journal_mode=%s", get_journal_mode_string(db_journal_mode));
	exec_sql(db[0], sql, NULL);
	sprintf(sql, "ATTACH DATABASE '%s/test.db7' AS 'db7'", pathname);
	exec_sql(db[0], sql, NULL);
	sprintf(sql, "PRAGMA db7.journal_mode=%s", get_journal_mode_string(db_journal_mode));
	exec_sql(db[0], sql, NULL);
	sprintf(sql, "ATTACH DATABASE '%s/test.db8' AS 'db8'", pathname);
	exec_sql(db[0], sql, NULL);
	sprintf(sql, "PRAGMA db8.journal_mode=%s", get_journal_mode_string(db_journal_mode));
	exec_sql(db[0], sql, NULL);
	...
	for(i = 0; i < db_transactions; i++) 
	{
		if(db_mode == 0)
		{
			sprintf(sql, "BEGIN; INSERT INTO tblMyList(id, Value) VALUES(%d, '%s'); INSERT INTO db1.tblMyList(id, Value) VALUES(%d, '%s'); INSERT INTO db2.tblMyList(id, Value) VALUES(%d, '%s'); INSERT INTO db3.tblMyList(id, Value) VALUES(%d, '%s'); INSERT INTO db4.tblMyList(id, Value) VALUES(%d, '%s'); INSERT INTO db5.tblMyList(id, Value) VALUES(%d, '%s'); INSERT INTO db6.tblMyList(id, Value) VALUES(%d, '%s'); INSERT INTO db7.tblMyList(id, Value) VALUES(%d, '%s'); INSERT INTO db8.tblMyList(id, Value) VALUES(%d, '%s'); COMMIT;", i, INSERT_STR, i, INSERT_STR, i, INSERT_STR, i, INSERT_STR, i, INSERT_STR, i, INSERT_STR, i, INSERT_STR, i, INSERT_STR, i, INSERT_STR);
			exec_sql(db[0], sql, NULL);
		}
		else if(db_mode == 1)
		{
			...
			sprintf(sql, "BEGIN; UPDATE tblMyList SET Value = '%s' WHERE id = %d; UPDATE db1.tblMyList SET Value = '%s' WHERE id = %d; UPDATE db2.tblMyList SET Value = '%s' WHERE id = %d; UPDATE db3.tblMyList SET Value = '%s' WHERE id = %d; UPDATE db4.tblMyList SET Value = '%s' WHERE id = %d; UPDATE db5.tblMyList SET Value = '%s' WHERE id = %d; UPDATE db6.tblMyList SET Value = '%s' WHERE id = %d; UPDATE db7.tblMyList SET Value = '%s' WHERE id = %d; UPDATE db8.tblMyList SET Value = '%s' WHERE id = %d; COMMIT;", UPDATE_STR, rand()%db_transactions + 1, UPDATE_STR, rand()%db_transactions + 1, UPDATE_STR, rand()%db_transactions + 1, UPDATE_STR, rand()%db_transactions + 1, UPDATE_STR, rand()%db_transactions + 1, UPDATE_STR, rand()%db_transactions + 1, UPDATE_STR, rand()%db_transactions + 1, UPDATE_STR, rand()%db_transactions + 1, UPDATE_STR, rand()%db_transactions + 1);
			exec_sql(db[0], sql, NULL);
		}
		else if(db_mode == 2)
		{
			//sprintf(sql, "DELETE FROM tblMyList WHERE id=%d;", i);
			sprintf(sql, "BEGIN; DELETE FROM tblMyList WHERE id=%d; DELETE FROM db1.tblMyList WHERE id=%d; DELETE FROM db2.tblMyList WHERE id=%d; DELETE FROM db3.tblMyList WHERE id=%d; DELETE FROM db4.tblMyList WHERE id=%d; DELETE FROM db5.tblMyList WHERE id=%d; DELETE FROM db6.tblMyList WHERE id=%d; DELETE FROM db7.tblMyList WHERE id=%d; DELETE FROM db8.tblMyList WHERE id=%d; COMMIT;", i, i, i, i, i, i, i, i, i);
			exec_sql(db[0], sql, NULL);
		}
		else
		{
	...
	{
		sqlite3_wal_checkpoint(db[0], NULL);
	}
	...
	for (i = 0; i < 9; i++) {
		if(db_mode == 2)
		{
			exec_sql(db[i], "DROP TABLE tblMyList;", NULL);
		}

		rc = sqlite3_close(db[i]);
