...
//	printf("open db : %s\n", filename);
	...
	for(i = 0; i < db_transactions; i++) 
	{
		...
		else if(db_mode == 1)
		{
			...
			sprintf(sql, "BEGIN; UPDATE tblMyList SET Value = '%s' WHERE id = %d; UPDATE db1.tblMyList SET Value = '%s' WHERE id = %d; UPDATE db2.tblMyList SET Value = '%s' WHERE id = %d; UPDATE db3.tblMyList SET Value = '%s' WHERE id = %d; UPDATE db4.tblMyList SET Value = '%s' WHERE id = %d ;COMMIT;", UPDATE_STR, rand()%db_transactions + 1, UPDATE_STR, rand()%db_transactions + 1, UPDATE_STR, rand()%db_transactions + 1,UPDATE_STR, rand()%db_transactions + 1,UPDATE_STR, rand()%db_transactions + 1);
			exec_sql(db[0], sql, NULL);
		}
		else if(db_mode == 2)
		{
			//sprintf(sql, "DELETE FROM tblMyList WHERE id=%d;", i);
			sprintf(sql, "BEGIN; DELETE FROM tblMyList WHERE id=%d; DELETE FROM db1.tblMyList WHERE id=%d; DELETE FROM db2.tblMyList WHERE id=%d; DELETE FROM db3.tblMyList WHERE id=%d; DELETE FROM db4.tblMyList WHERE id=%d; COMMIT;", i, i, i, i, i);
			exec_sql(db[0], sql, NULL);
		}
		else
		{
