...
//	printf("open db : %s\n", filename);
	...
	for(i = 0; i < db_transactions; i++) 
	{
		...
		else if(db_mode == 2)
		{
			...
			sprintf(sql, "BEGIN; DELETE FROM tblMyList WHERE id=%d; DELETE FROM db1.tblMyList WHERE id=%d; DELETE FROM db2.tblMyList WHERE id=%d; DELETE FROM db3.tblMyList WHERE id=%d; DELETE FROM db4.tblMyList WHERE id=%d; DELETE FROM db5.tblMyList WHERE id=%d; DELETE FROM db6.tblMyList WHERE id=%d; COMMIT;", i, i, i, i, i, i, i);
			exec_sql(db[0], sql, NULL);
		}
		else
		{
