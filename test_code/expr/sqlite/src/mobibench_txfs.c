...
int thread_main_db(void* arg)
{
	...
	int i,j;
	int column_count = 0;
	char* sql = (char*)malloc(sizeof(char)*100*1000*1000);
	memset(sql,0,sizeof(char)*100*1000*1000);
	struct stat statbuf;
	int length = 0;
	char * random_check;
	if(num_threads == 1)
...
//	printf("open db : %s\n", filename);
	...
	for(i = 0; i < db_transactions; i++) 
	{
		...
		length+=sprintf(sql,"BEGIN;");
			...
			srand(i);
                        for(j=0; j<numberOfTable; j++){
                                length += sprintf(sql+length, "UPDATE tblMyList%d SET Value = '%s' WHERE id = %d;", j, UPDATE_STR, rand()%db_transactions);
                        }
                        strcat(sql,"COMMIT;");
                        exec_sql(db,sql,NULL);
                }
		...
		{
			for(j=0; j<numberOfTable ; j++){
				length+=sprintf(sql+length, "DELETE FROM tblMyList%d WHERE id=%d;", j, i);
			}
			strcat(sql,"COMMIT;");
			exec_sql(db, sql, NULL);
		}
