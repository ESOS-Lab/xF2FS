			...
			struct writeback_control *wbc, bool last_file, struct atomic_files_header *af_header)
{
	...
	printk(KERN_DEBUG "[MUFIT DEBUG] Syncing node pages\n");
#endif
...
#ifdef F2FS_MFAW_DEBUG
				printk(KERN_DEBUG "[MUFIT DEBUG] Set prev_atmaddr: %lX\n", af_header->prev_atmaddr);
#endif

				// Store current blkaddr to af_header.
				if(last_file) {
#ifdef F2FS_MFAW_DEBUG
					printk(KERN_DEBUG "[MUFIT DEBUG] set fsync mark\n");
#endif
					set_fsync_mark(page, 1);
/*				} else {
...
#ifdef F2FS_MFAW_DEBUG
				printk(KERN_DEBUG "[MUFIT DEBUG] Current node address: %lX\n", af_header->prev_atmaddr);
#endif
