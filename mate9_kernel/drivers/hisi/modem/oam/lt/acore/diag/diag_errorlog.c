...
VOS_VOID OM_AcpuRcvMsgFinish(VOS_VOID)
{
    ...
    (VOS_VOID)VOS_MemSet(&stOmAppReportStatus, 0, sizeof(stOmAppReportStatus));
    ...
    VOS_MemSet(g_ucErrRptFlag,OM_MSG_RECEIVE_FLAG,sizeof(g_ucErrRptFlag));
    /* 防止消息再次下发不成功 */
...
VOS_VOID OM_AcpuSendAppResult(VOS_UINT32 ulRest, VOS_UINT16  usModemId)
{
    ...
    (VOS_VOID)VOS_MemSet(&stOmAppResultCnf, 0, sizeof(stOmAppResultCnf));
...
VOS_INT OM_AcpuReportErrLogMsg(VOS_UINT8 *pucData, VOS_UINT32 ulLen)
{
    ...
    /* 根据告警相关性，向对应PID发送消息 */
    (void)vos_printf("OM_AcpuReportErrLogMsg: alarmid:0x%x!\n",g_stOmAppMsgRecord.ulAlarmIdNum);
                          ...
                          VOS_UINT8  ucAlmLevel)
{
    ...
    (VOS_VOID)VOS_MemSet(&stAppOmCtrlStatus, 0, sizeof(stAppOmCtrlStatus));
