...
)
{
      ...
      TAF_MEM_SET_S(aucSpeed, sizeof(aucSpeed), 0x00, (VOS_SIZE_T)(AT_AP_SPEED_STRLEN + 1));
...
)
{
    ...
    if ((0 == ulNvDialRateIndex) || (ulNvDialRateIndex > 6))
    {
          ...
          || (TAF_PH_INFO_TD_SCDMA_RAT == stSysMode.enRatType) )
        {
            ...
            TAF_MEM_CPY_S(aucDialRateTmp, (VOS_SIZE_T)sizeof(aucDialRateTmp), g_PppDialRateDisplay[enRateDisplay], ulTmp);
        ...
        else if (TAF_PH_INFO_LTE_RAT == stSysMode.enRatType)
        {
            ...
            TAF_MEM_CPY_S(aucDialRateTmp, (VOS_SIZE_T)sizeof(aucDialRateTmp), g_ucLTEDlRateDisplay[enRateDisplay], ulTmp);
        }
        else
        {
    ...
    else
    {
        ...
        TAF_MEM_CPY_S(aucDialRateTmp, (VOS_SIZE_T)sizeof(aucDialRateTmp), g_ucDialRateDisplayNv[ulNvDialRateIndex - 1], ulTmp);
    }

    /*在CONNECT后附上速率信息*/
    TAF_MEM_CPY_S(ucSpeed, AT_AP_SPEED_STRLEN + 1, aucDialRateTmp, (VOS_UINT16)ulTmp);
    ucSpeed[ulTmp] = '\0';
