...
)
{
    ...
    PSACORE_MEM_CPY(pSendMsg, ulTransMsgContentLen, pTransMsgContent, ulTransMsgContentLen);
...
)
{
    ...
    PSACORE_MEM_CPY(pstTraceMsg->aucNetIfName, IPS_IFNAMSIZ, pucNetIfName, IPS_IFNAMSIZ);

    /* 在某些情况下(如配置 pstTraceCfg->ulTraceDataLen 为0)，ulDataLen为0的情况，存在异常，这里进行判断，保护这种情况。*/
    if ( 0 != ulDataLen )
    {
        /*lint -e669 */
        PSACORE_MEM_CPY(pstTraceMsg->aucData, ulDataLen, pucPktData, ulDataLen);
        /*lint +e669 */
    }    
...
)
{
    ...
    {
        PSACORE_MEM_CPY(pstTraceMsg->aucSrcPort, IPS_END_NAME, pucSrcPort, IPS_END_NAME);
    }
    if (VOS_NULL_PTR != pucDestPort)
    {
        PSACORE_MEM_CPY(pstTraceMsg->aucDestPort, IPS_END_NAME, pucDestPort, IPS_END_NAME);
    }
