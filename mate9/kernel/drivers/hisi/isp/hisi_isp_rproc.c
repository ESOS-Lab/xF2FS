...
int hisp_nsec_jpeg_powerup(void)
{
    ...
    pr_info("%s: +\n", __func__);
    if (!ops) {
        pr_err("%s: failed, isp_ops.0x%pK is null.\n", __func__, ops);
		return -EINVAL;
    }
...
static int hisi_isp_nsec_getdts(struct platform_device *pdev, struct hisi_isp_nsec *dev)
{
    ...
    if (IS_ERR(dev->ispsrt_supply)) {
        pr_err("[%s] Failed : devm_regulator_get.%pK\n", __func__, dev->ispsrt_supply);
        return -EINVAL;
    }
...
static int isp_remap_rsc(struct hisi_isp_nsec *dev)
{
	...
	pr_info("%s: crgperi_base.0x%pK, vivobus_base.0x%pK, isp_regs_base.0x%pK, isp_dma_va.0x%pK, dma.0x%llx\n", __func__,
