...
{
    int i = 0;
	HI_IPF_CH0_ADQ_CTRL_T adq_ctrl0;
#ifndef CONFIG_PSAM
	HI_IPF_CH1_ADQ_CTRL_T adq_ctrl1;
#endif
    struct ipf_share_mem_map* sm = bsp_ipf_get_sharemem();

	// disable ch0 adq
    adq_ctrl0.u32= ipf_readl(g_ipf_ctx.regs + HI_IPF_CH0_ADQ_CTRL_OFFSET);
	adq_ctrl0.bits.ul_adq_en = 0;
	ipf_writel(adq_ctrl0.u32, g_ipf_ctx.regs + HI_IPF_CH0_ADQ_CTRL_OFFSET);

#ifndef CONFIG_PSAM
    // disable ch1 adq
    adq_ctrl1.u32 = ipf_readl(g_ipf_ctx.regs + HI_IPF_CH1_ADQ_CTRL_OFFSET);
    adq_ctrl1.bits.ul_adq_en = 0;
	ipf_writel(adq_ctrl0.u32, g_ipf_ctx.regs + HI_IPF_CH1_ADQ_CTRL_OFFSET);
#endif
    
    for(i=0; i<sizeof(ipf_save_table)/sizeof(ipf_save_table[0]); i++)
    {
       sm->reg_save[i] = ipf_readl(ipf_save_table[i] + g_ipf_ctx.regs);
    }
...
static int _ipf_reg_load(void)
{
    ...
    for(i=0; i<sizeof(ipf_save_table)/sizeof(ipf_save_table[0]); i++)
    {
       ipf_writel(sm->reg_save[i],ipf_save_table[i] + g_ipf_ctx.regs);
    }
...
static int ipf_pm_prepare_ex(struct device *pdev)
{
    ...
    HI_IPF_CH_EN_T ch_en;
    struct ipf_share_mem_map* sm = bsp_ipf_get_sharemem();

    /* ÅÐ¶ÏÉÏÐÐIPFÊÇ·ñ¿ÕÏÐ */
    u32_ul_state = ipf_readl(g_ipf_ctx.regs + HI_IPF_CH0_STATE_OFFSET);
    ...
    ch_en.u32= ipf_readl(g_ipf_ctx.regs + HI_IPF_CH_EN_OFFSET);
    ch_en.bits.ul_en = 0;
	ipf_writel(ch_en.u32, g_ipf_ctx.regs + HI_IPF_CH_EN_OFFSET);
...
static int ipf_pm_resume_ex(struct device *pdev)
{
	...
	status = ipf_readl(g_ipf_ctx.regs + HI_IPF_INT0_OFFSET); 
	...
	reg = ipf_readl(g_ipf_ctx.regs + HI_IPF_CH_EN_OFFSET);
	ch_en_p = (HI_IPF_CH_EN_T*)&reg;
    ch_en_p->bits.ul_en = 1;
	ipf_writel(reg, g_ipf_ctx.regs + HI_IPF_CH_EN_OFFSET);
